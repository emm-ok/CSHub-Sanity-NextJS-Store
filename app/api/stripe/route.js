...
export async function POST(req) {
  try {
    ...
    const line_items = items.map((item) => {
      ...
      const [prefix, id, dimension, format] = img.split("-");

      const newImage = `https://cdn.sanity.io/images/h68r87qb/production/${id}-${dimension}.${format}`;
    ...
    return new Response(
      JSON.stringify({ url: session.url, selectedItems: items }),
      {
        status: 200,
